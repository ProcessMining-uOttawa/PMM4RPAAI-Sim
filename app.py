...
# --- Two top-level flows -----------------------------------------------------
# The experiment pipeline (numbered panels) and the model-fidelity check are
# distinct flows sharing one sidebar, split into tabs rather than pages: every
# tab body runs on each rerun (hidden via CSS, not unmounted), so widget state
# and the fidelity poller survive tab switches. The fidelity block renders
# first so an experiment-side st.stop() cannot blank it.
experiment_tab, fidelity_tab = st.tabs(["Experiment", "Model fidelity"])

with fidelity_tab:
    render_fidelity_panel(ss, n_reps, max_workers, demo_mode)
