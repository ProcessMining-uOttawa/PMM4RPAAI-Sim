...
- **Parallel simulation — configurable worker count**: exposed as "Parallel workers" in the run config sidebar (`min=1`, `max=os.cpu_count()`, `default=1`). Propagates from `app.py` → `orchestrator.run_experiment(max_workers=...)` → `run_all(max_workers=...)`. Default of 1 preserves sequential behaviour; users raise it deliberately. Demo mode is single-threaded and does not use this parameter.
