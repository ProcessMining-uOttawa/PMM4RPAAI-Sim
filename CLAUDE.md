...
| [core/orchestrator.py](core/orchestrator.py) | Real-pipeline run loop only: pre-generates all scenario JSONs sequentially, then delegates parallel execution to `simulation.pool.run_all()`, collects results into a tidy DataFrame. Also runs the baseline (original, untransformed model) and returns mean total metrics + rework means for Panel 5. No demo awareness — `app.py` routes to `demo.run_experiment()` for demo mode. |
...
| [core/simulation/pool.py](core/simulation/pool.py) | Parallel simulation executor. `SimulationTask` dataclass: `bpmn_path`, `json_path`, `n_cases`, `out_log`, `out_stat`, `proc_log`, `metadata`. `TASK_BASELINE`/`TASK_SCENARIO` string constants discriminate the two task kinds (used by `orchestrator.py` in metadata tuples). `run_all(tasks, on_complete, max_workers=None)` submits all tasks to a `ThreadPoolExecutor` (`max_workers` defaults to `os.cpu_count()`); `on_complete` is always called on the calling thread via `as_completed`, so callers can mutate shared state without locks. Fails fast via `pool.shutdown(cancel_futures=True)` on first simulation failure. `subprocess.run()` in `runner.simulate` releases the GIL, giving true OS-level parallelism. |
...
| [ui/goals.py](ui/goals.py) | `GoalOption` NamedTuple (`default_pct`, `default_weight`, `step`, `allow_zero`) and `GOAL_OPTIONS: dict[Metric, GoalOption]` keyed by `MetricRegistry` metric singletons. The sidebar renders a 3-column table (metric name / reduction % / weight) for all three rankable metrics. The goal column is read from `metric.per_case.mean.column` at the call site. |
...
- **Multi-goal weighted ranking** (`core/analysis.py`, `core/goals.py`, `ui/goals.py`, `app.py`): `rank(agg, goals)` accepts all three goals simultaneously, each with an independent percentage-reduction target and weight. `score = Σ weight_i × (metric_i / target_i)` — lower is better; `goal_met = True` only when every non-zero-weight goal is individually satisfied. Zero-weight goals are excluded from both score and `goal_met`, letting users effectively disable a goal without removing it from the UI. Targets are percentage reductions from baseline — converted to absolute thresholds at ranking time via `Goal.from_pct_reduction()`, which requires both results and baseline to be available (they always are at ranking time). Weights sum to ~1 by convention; no enforcement gate is applied — the score is still well-defined if they don't. `baseline_per_case()` in `core/goals.py` derives per-case baseline values from the orchestrator's `baseline_agg`; `demo.demo_baseline_agg()` returns the same shape so `app.py` has a single call site: `baseline_per_case(ss.baseline_agg or demo.demo_baseline_agg())`. Deferred: (1) pct bounds validation (0–99%, since 100% → zero target → division by zero); (2) weight sum display or normalization; (3) showing the goals UI only after an event log is uploaded.
...
- **Cancel mid-run**: the discovery cancel works; the simulation run loop
  doesn't yet check a session-state flag between iterations. Cancellation would need to signal the pool (e.g. a threading.Event checked between task submissions) and is kept separate from the parallelism work.
...
- **`TASK_BASELINE`/`TASK_SCENARIO` constants placement**: currently defined in `pool.py` but only used in `orchestrator.py` (`pool.py` never inspects `task.metadata`). These constants encode the orchestrator's tagging scheme, not the pool's execution logic — they belong in `orchestrator.py`. Deferred until recovery lands (at which point the whole metadata contract will be revisited).
