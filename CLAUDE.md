...
- **Flexible goal selection** (`app.py`, `ui/goals.py`): users can enable any 1–3 subset of goals via a `st.multiselect` above the pct/weight table rather than always ranking against all three. Default is all three (no behaviour change for existing users). Options come from `GOAL_OPTIONS.keys()` with `format_func=lambda m: m.per_case_display_name` — no hardcoded display strings. `_active = [m for m in GOAL_OPTIONS if m in set(_selected)]` re-sorts by `GOAL_OPTIONS` insertion order regardless of click order so the table order is stable. The pct/weight header, per-metric rows, and weight-sum warning are all gated on `_active` being non-empty — prevents the false "weights sum to 0.00" warning when all goals are cleared. An empty-selection caption ("No goals selected — scenarios will not be ranked.") is shown instead. `rank()` already handles `goals=[]` gracefully — no change to the analysis layer. All `st.xxx()` calls stay in `app.py`, consistent with the pure-function convention in `ui/table.py` and `ui/plots.py`; `ui/goals.py` remains config-only.
