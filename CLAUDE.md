...
- **`test_analysis.py` missing rework coverage**: `aggregate()`, `compare_to_baseline()`, `main_effects()`, and `rank()` are all tested but none of the fixtures carry rework columns. The `_results_df()` helper has no rework data, so `COL_REWORK_COUNT_MEAN`/`COL_REWORK_RATE_MEAN` aggregation, the rework delta computations in `compare_to_baseline`, and `main_effects(results, COL_REWORK_RATE)` are uncovered.
...
- **`compare_to_baseline` column structure** (`core/analysis.py`): the baseline and scenario rows are built by manually repeating a `b_x = b.get(...)` / `s_x = row.get(...)` / `d_x = s_x − b_x` block for each metric (cycle, cost, rework count, rework rate). A data-driven alternative — iterating a list of `(col_mean, display_label, unit_fn)` descriptors — would halve the line count and mean adding a new metric is a one-line data change rather than eight scattered edits across two blocks. Kept manual at the current three metrics because the abstraction adds indirection without saving much. Revisit if a fourth display metric is added.
