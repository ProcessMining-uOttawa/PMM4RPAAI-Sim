...
- **Panel 4 ranked table — remaining column readability**: factor columns are already renamed via `{p.id: p.label for p in params}` at render time. Two further categories remain:
  - **KPI metric columns** (`cycle_h_mean`, `cost_mean`, `rework_rate_mean`, etc.): need a display-name mapping. `_METRICS` in `analysis.py` already carries labels ("Total Cycle Time (h)", "Total Cost ($)", …) and could be the source of truth — a `{m.col: m.label}` dict derived from it avoids a second copy of the labels. `cycle_h_std` / `cost_std` are not in `_METRICS`; decide whether to hide them or give them ad-hoc labels.
  - **Scenario ID column**: rename header from `"scenario_id"` to `"Scenario"`. On stripping the `S` prefix from values (`S01` → `1`): the `rank` column already shows performance order as integers, so bare integers for scenario identity risk being read as a second rank. Preferred: keep the `S` prefix but shorten to `"S1"`, `"S2"` (drop zero-padding) — or keep zero-padded and just fix the header. Decide before implementing.
