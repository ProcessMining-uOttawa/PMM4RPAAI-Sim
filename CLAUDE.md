...
- **Main-effects chart in `ui/plots.py`** (`ui/plots.py`, `app.py`): chart logic lives in a dedicated `ui/plots.py` module (two public functions: `factor_label_map` and `main_effects_chart`). The module is justified by the complexity of the chart function today — layout config, facet title cleanup, categorical axis handling — not by anticipated future reuse (preemptive organisation, not preemptive abstraction). The chart plots `mean` on the Y-axis only; `sn` (Signal-to-Noise ratio) is intentionally omitted from display because it has NaN gaps for zero rework rate (see §8 known bug), but it remains in `main_effects()` output so it can be surfaced once the `floor` fix lands. The `factor` column values in `main_effects()` output are raw IDs prefixed by the target activity name (e.g. `"Fix Bug.pct_auto"`); `factor_label_map` translates these to `Parameter.label` strings (e.g. `"Automation rate (%)"`) using the `params` list already in scope at the call site — no string-stripping required. `level` values are converted to strings before passing to Plotly to ensure even categorical spacing on the X-axis regardless of whether the numeric levels are evenly spaced. The `st.dataframe` tables in the three main-effects tabs in Panel 4 are replaced entirely by `st.plotly_chart` — the chart conveys the same information more compactly, and keeping both would make the panel excessively long.
...
- **Plots in Panel 4**: replace the three `st.dataframe` main-effects tables with Plotly faceted line charts (one chart per goal tab). Design decisions are recorded above. Implementation: new `ui/plots.py` with `factor_label_map` + `main_effects_chart`; `app.py` swaps `st.dataframe` for `st.plotly_chart`; `tests/test_plots.py` covers both functions. Branch: `feature/main-effects-plot`.
