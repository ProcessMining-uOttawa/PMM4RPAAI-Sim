...
| [ui/goals.py](ui/goals.py) | `GoalOption` NamedTuple (`default_pct`, `default_weight`, `step`, `allow_zero`) and `GOAL_OPTIONS: dict[Metric, GoalOption]` keyed by `MetricRegistry` metric singletons. The sidebar renders a 3-column table (metric name / reduction % / weight) for all three rankable metrics. The goal column is read from `metric.per_case_column` at the call site. |
...
- **Reset log while run is in-flight** (`app.py`): the "Reset log" button clears results-derived session state (`results`, `experiment_bpmn_path`, `scenario_json_paths`, `baseline_agg`, etc.) but does NOT clear `run_state`, `run_thread`, or `stop_event`. If the user resets while a background run is still live, the thread completes normally and `_panel3()` writes its result back into `ss.results` and friends — silently undoing the reset. The fix is to call `cancel_experiment(ss)` before clearing state, then wait (or ignore) the cancelled result. Not yet implemented because the interaction is unlikely in normal use and the fix requires careful ordering to avoid the `clear_run()` / `st.rerun()` race in `_panel3()`.
- **Session state as implicit state machine** (`app.py`): the app has four implicit states — IDLE (no log), DISCOVERED (log + activities), RUNNING (background thread alive), DONE (results present) — but transitions between them are spread across the script as ad-hoc key resets. Every time a new session-state key is added (e.g. `scenario_log_paths`), it must be manually added to the reset block or it silently persists across resets. The lightweight fix: two helper functions `_clear_results()` (clears all run-level keys: `results`, `experiment_bpmn_path`, `scenario_json_paths`, `baseline_agg`, `scenario_log_paths`, `baseline_log_paths`) and `_clear_log()` (calls `_clear_results()` then clears log-level keys: `log_name`, `log_path`, `activities`, `bpmn_path`, `json_path`, `log_fingerprint`). New keys are added to one helper, not scattered. A formal FSM library would be overkill for four states.
