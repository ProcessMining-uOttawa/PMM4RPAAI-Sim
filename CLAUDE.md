...
- **Rework KPI — Panel 4 column visibility**: rework columns (`Rework Count`, `Δ Rework Count`, `Δ Rework (%)`, `Rework Rate (%)`, `Δ Rate (pp)`) surface in Panel 5 and the ranking table in Panel 4. The rework rate goal option and main-effects tab are implemented. Column readability (renaming, std hiding) is handled by the `MetricRegistry` rename map — see §8 design decision "Panel 4 ranked table column readability."
