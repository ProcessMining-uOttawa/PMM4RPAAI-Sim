"""Tests for pool.run_all() — stop_check predicate, completion flag, and on_error callback."""
from __future__ import annotations

from core.simulation import pool, runner


def _tasks(tmp_path, n: int, max_retries: int = 0) -> list[pool.SimulationTask]:
    return [
        pool.SimulationTask(
...
class TestRunAllOnError:
    ...
    def test_on_error_called_when_simulate_raises(self, monkeypatch, tmp_path):
        ...
        errors: list = []
        pool.run_all(_tasks(tmp_path, 2), lambda t: None, on_error=lambda t, e: errors.append(t))
        assert len(errors) == 2
    ...
    def test_no_on_error_provided_failures_silently_skipped(self, monkeypatch, tmp_path):
        monkeypatch.setattr(runner, "simulate", _always_fail)
        result = pool.run_all(_tasks(tmp_path, 2), lambda t: None)
        assert result is True
